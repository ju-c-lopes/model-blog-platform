...
def view_author_page(request, slug):
    ...
    if request.user != author.user:
        return render(
            request, template_name="blog/pages/author/author.html", context=context, status=200
        )
...
def edit_author_profile(request, slug):
    author = get_object_or_404(Author, author_url_slug=slug)
    author_social_media = SocialMedia.objects.filter(author=author)
    ...
    if request.POST:
        ...
        if forms_ok:
            ...
            return render(
                request,
                template_name="blog/pages/edit-author/edit-author.html",
                context=context,
            )
    else:
    ...
    return render(
        request, template_name="blog/pages/edit-author/edit-author.html", context=context
    )
        ...
        author_post_request_data = {
            ...
            > len(
                SocialMedia.objects.filter(
                    author__user__username=request.POST["username"]
                )
            ),
...
def create_social_media(request, author_request_post):
    ...
    social = SocialMedia.objects.filter(author=author)
