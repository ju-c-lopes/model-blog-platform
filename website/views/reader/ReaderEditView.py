...
def reader_edit(request):
    ...
    return render(
        request, template_name="blog/pages/edit-reader/edit-reader.html", context=context
    )
...
def check_request_post(request):
    ...
    if request.POST:
        reader_post_request_data = {
            ...
            "check_username_request": Reader.objects.filter(
...
def check_user_form(request, reader):
    ...
    if (
        user_form.is_valid()
        and reader_user["username"] != reader.user.username
        and username_free
    ):
